...
def get_all_contacts():
    ...
    try:
        with open('GPyS7H/contacts.csv') as csvfile:
            ...
            for row in reader:
                contact=Record(row[0],row[1],row[2],row[3])
                res.append(contact)
...
def find_by(var,fstr):
    ...
    for contact in contacts:
        if ((var==1 and fstr in contact.number) or (var==2 and fstr in (contact.firstname or contact.lastname))):
            res.append(Record(contact.number,contact.firstname,contact.lastname,contact.description))
    return res
...
def import_from_json():
    try:
        ...
        for i in data['contacts']:
            write_file(Record(i['number'],i['firstname'],i['lastname'],i['description']))
    except EnvironmentError:
        return -1
    return 1
...
def import_from_xml():
    try:
        ...
        for contact in root.findall('contact'):
            ...
            write_file(Record(number,firstname,lastname,description))
    except EnvironmentError:
        return -1
    return 1